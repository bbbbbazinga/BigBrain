...
describe('BarCharts component', () => {
  ...
  describe('pass null to prpos', () => {
    it('to test each Bar', () => {
      const wrapper = shallow(<BarChart correctPer={[]} avgTime={[]} />);
      expect(wrapper.childAt(0).props().data.labels).toStrictEqual([]);
      expect(wrapper.childAt(0).props().options).toBeDefined();
      expect(wrapper.childAt(1).props().data.labels).toStrictEqual([]);
      expect(wrapper.childAt(1).props().options).toBeDefined();
    });
  });
});
